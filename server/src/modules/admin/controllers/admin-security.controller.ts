...
export const adminSecurityController = {
  ...
  async loginActivity(req: Request, res: Response) {
    ...
    const activity = logs.map((l) => {
      const meta = (() => {
        try {
          return l.metadata ? (JSON.parse(l.metadata) as Record<string, unknown>) : {};
        } catch {
          return {};
        }
      })();
  ...
  async failedAttempts(_req: Request, res: Response) {
    ...
    const map = new Map<string, { ip: string; attempts: number; lastAttempt: Date; user: string | null }>();
    for (const l of logs) {
      let meta: any = {};
      try {
      ...
      if (existing) {
        existing.attempts += 1;
        if (l.createdAt > existing.lastAttempt) existing.lastAttempt = l.createdAt;
      } else {
  ...
  async listIpRules(_req: Request, res: Response) {
  ...
  async createIpRule(req: Request, res: Response) {
  ...
  async deleteIpRule(req: Request, res: Response) {
